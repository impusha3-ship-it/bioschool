 ...
 *
 * Осторожно: эти числа управляют не только начислением, но и порогами значков
 * ниже. Значок «Без ошибок» опознаёт чистую игру по цене 15 (10 + 5), и если
 * поменять здесь цену игры, у всех учеников разом пропадут уже показанные
 * значки — молча, без ошибки и без единого следа в логе. Меняя цифру, считай,
 * что меняешь прошлое, а не только будущее.
 */
export const НОРМЫ = {
  игра: { за: 10, чисто: 5 },
 ...
 *
 * `weeks` — все баллы недели, `hwWeeks` — только домашние. Два счётчика, а не
 * один, потому что считают они разное: по `weeks` живут серия недель и значок
 * «Ударная неделя» — там важно, занимался ли ученик вообще; по `hwWeeks`
 * строится таблица почёта, а в неё идут только сданные работы.
 */
export const ПУСТО = { v: 1, lessons: {}, weeks: {}, hwWeeks: {}, lastSeen: 0 };
...
export function начислить(состояние, событие, дата = new Date()) {
  ...
  if (добавлено > 0) {
    ...
    if (kind === 'homework') s.hwWeeks[w] = (s.hwWeeks[w] ?? 0) + добавлено;
  }
...
/**
 * Баллы за сданные домашние работы — то, и только то, по чему строится
 * таблица почёта.
 *
 * Почему не общая сумма: тренажёр открыт всем и всегда, домашку задаёт
 * учитель классу и сдаётся она один раз. В уроке с двумя играми,
 * лабораторной и блоком ВПР мимо домашки лежит 45 баллов против 30 за саму
 * работу — считай сумму, и впереди окажется тот, у кого больше свободного
 * времени, а не тот, кто лучше учится. Решение учителя от 16 сентября 2026.
 *
 * Отдельно нигде не хранится: домашние баллы урока и так лежат под ключом
 * `homework`, а вторая копия того же числа однажды разойдётся с первой.
 */
export function xpДомашек(состояние) {
  return Object.values(состояние?.lessons ?? {})
    .reduce((сумма, урок) => сумма + число(урок?.homework), 0);
}
...
export const ЗНАЧКИ = [
  ...
  { id: 'lesson-perfect', имя: 'Урок на отлично', условие: 'Пройди игры, лабораторную и блок ВПР одного урока без ошибок',
    есть: (s) => Object.values(s.lessons ?? {}).some(идеальный) },
  { id: 'five-homework', имя: 'Отличник', условие: 'Сдай пять домашних работ на девяносто и выше',
    есть: (s) => отличныхДомашек(s) >= 5,
    счёт: (s) => `${Math.min(отличныхДомашек(s), 5)} из 5` },
  { id: 'week-100', имя: 'Ударная неделя', условие: 'Набери сто баллов за одну неделю',
    есть: (s) => Object.values(s.weeks ?? {}).some((xp) => число(xp) >= 100) },
  { id: 'three-clean', имя: 'Твёрдая рука', условие: 'Пройди три игры без единой ошибки',
    есть: (s) => чистыхИгр(s) >= 3,
    счёт: (s) => `${Math.min(чистыхИгр(s), 3)} из 3` },
  { id: 'year', имя: 'Год закрыт', условие: 'Пройди все уроки класса',
    // Пустой курс не считается закрытым: every() на пустом списке истинно, и
    // класс, до которого ещё не написано ни одного урока, выдал бы значок за
    // ничто. Та же защита стоит у «Раздела закрыт».
    есть: (s, курсы) => курсы.some((к) => {
      const уроки = урокиКурса(к);
      return уроки.length > 0 && уроки.every((id) => s.lessons[id]?.done);
    }) },
];
...
/**
 * Потолок для вида работы — сколько даёт безупречный результат.
 *
 * Считается из тех же НОРМ, что и начисление: разъедься эти два места, и
 * значок «на отлично» перестал бы выдаваться там, где ученик всё сделал
 * верно, а понять почему было бы неоткуда.
 */
function потолок(kind) {
  if (ИГРОВОЙ_ВИД.test(kind)) return ЧИСТАЯ_ИГРА;
  if (kind === 'lab') return ЧИСТАЯ_ЛАБА;
  if (kind === 'vpr') return НОРМЫ.впр.потолок;
  if (kind === 'homework') return НОРМЫ.домашка.за + НОРМЫ.домашка.отлично;
  return null;
}

/**
 * Урок, взятый на максимум по всей работе внутри самого урока.
 *
 * Требуется каждый вид, а не сумма: урок с чистой игрой и половиной блока
 * ВПР суммой обгонял бы урок, сделанный ровно, — и значок ушёл бы не тому.
 * Незаконченный урок не считается вовсе.
 *
 * Домашка сюда не входит намеренно. Сдать её можно один раз и только если
 * учитель задал урок этому классу: одна работа на 89 закрыла бы значок по
 * этому уроку навсегда, а гость не получил бы его никогда. Значок должен
 * зависеть от того, что ученик делает сам, а не от того, что ему задали.
 */
function идеальный(урок) {
  if (!урок?.done) return false;
  const виды = Object.entries(урок)
    .filter(([k, v]) => typeof v === 'number' && k !== 'homework');
  return виды.length > 0 && виды.every(([k, v]) => потолок(k) !== null && v === потолок(k));
}

function отличныхДомашек(состояние) {
  return Object.values(состояние.lessons ?? {})
    .filter((урок) => урок.homework === потолок('homework')).length;
}

function чистыхИгр(состояние) {
  return Object.values(состояние.lessons ?? {}).reduce(
    (n, урок) => n + Object.entries(урок)
      .filter(([k, v]) => ИГРОВОЙ_ВИД.test(k) && v === ЧИСТАЯ_ИГРА).length,
    0,
  );
}
