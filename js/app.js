...
import { createВерсия, перезагрузитьСвежим } from './versiya.js';
...
/*
  Версия сайта запоминается при открытии вкладки и сверяется в фоне. Если
  вышла новая, перезагружаемся на ближайшем переходе: переход ученик делает
  сам, и то, что он вписывал, в этот момент уже не на экране.

  Одна брешь остаётся: если вкладку открыли в первые десять минут после
  выкладки, код мог приехать из кеша браузера старым, а отпечаток — уже
  новым, и такая вкладка обновится только со следующей выкладкой или при
  перезагрузке. Закрыть её без сборки сайта нечем.
*/
const версия = createВерсия();
версия.запомнить();
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'visible') версия.проверить();
});

startRouter(async (route) => {
  if (версия.устарела() && версия.можноПерезагрузить()) {
    версия.отметитьПерезагрузку();
    mount.replaceChildren(el('p', { class: 'loading' }, 'Сайт обновился — загружаю новую версию…'));
    перезагрузитьСвежим();
    return;
  }
  версия.проверить();
