...
export function questionField(q, ответы = {}, настройки = {}) {
  ...
  // Ключ к развёрнутому заданию домашки — только учителю. Раньше то же самое
  // стояло в подсказке, и ученик читал ответ прямо под вопросом.
  if (key && q.type === 'open' && q.answerKey) {
    тело.push(e('p', { class: 'q__key' }, `Ключ: ${q.answerKey}`));
  }
