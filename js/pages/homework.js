...
function собратьРаботу(lesson, сессия, назначение, блок) {
  ...
  /*
    Черновик пишется на каждый ответ, а не только при сбое входа.

    Вкладки урока — это адреса, и переход на «Конспект» и обратно собирает
    домашку заново: объект ответов при этом создаётся пустым. До 23 сентября
    2026 черновик сохранялся лишь в одном случае — когда вход устарел прямо
    посреди работы, — и ученик, сходивший свериться с конспектом, возвращался
    к чистому бланку. Теперь любое возвращение на вкладку поднимает ответы.

    Ходы игры так не поднимаются: у игр нет ни чтения, ни записи состояния,
    и приделывать их здесь было бы правкой вслепую по шести видам игр.
  */
  const ответы = прочитатьЧерновик(lesson.id) ?? {};
  const запомнить = () => сохранитьЧерновик(lesson.id, ответы);
  const части = [];
  ...
  for (const q of вопросы.filter(isAuto)) {
    части.push(questionField(q, ответы, { наИзменение: запомнить }).element);
  }
  ...
  кнопка.addEventListener('click', async () => {
    ...
    try {
      ...
      if (error.code === ВХОД_УСТАРЕЛ) {
        // Черновик и так пишется на каждый ответ; здесь важно, что игру
        // после повторного входа придётся пройти заново: её ходы не хранятся.
        запомнить();
