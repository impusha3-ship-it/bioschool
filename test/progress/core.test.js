...
import { ПУСТО, начислить, ценность, всегоXp, xpДомашек, ступень, серия, СТУПЕНИ } from '../../js/progress/core.js';
...
/*
  Таблица почёта считается только по домашним работам, и это единственное
  место, где баллы разделены. Тренажёр открыт всем и всегда, домашку задаёт
  учитель и сдаётся она один раз: сложи всё вместе — и впереди окажется тот,
  у кого больше свободного времени. Остальной прогресс при этом считает всё
  по-прежнему, поэтому обе суммы проверяются рядом.
*/
test('в домашние баллы идёт только домашка', () => {
  let s = начать();
  s = начислить(s, { lessonId: 'у1', kind: 'game0', correct: 8, total: 8, состав: [] }, ДАТА).состояние;
  s = начислить(s, { lessonId: 'у1', kind: 'lab', correct: 3, total: 3, состав: [] }, ДАТА).состояние;
  s = начислить(s, { lessonId: 'у1', kind: 'homework', percent: 100, состав: [] }, ДАТА).состояние;

  assert.equal(xpДомашек(s), 30, 'домашка на сто процентов — двадцать плюс десять');
  assert.equal(всегоXp(s), 65, 'общий счёт по-прежнему считает игру и лабораторную');
});

test('домашние баллы недели копятся отдельно от общих', () => {
  let s = начать();
  s = начислить(s, { lessonId: 'у1', kind: 'game0', correct: 8, total: 8, состав: [] }, ДАТА).состояние;
  s = начислить(s, { lessonId: 'у1', kind: 'homework', percent: 50, состав: [] }, ДАТА).состояние;

  assert.equal(s.weeks['2026-W34'], 35, 'в общую неделю вошло всё');
  assert.equal(s.hwWeeks['2026-W34'], 20, 'в домашнюю — только работа');
});

test('без домашек домашний счёт нулевой, а не пустой', () => {
  const s = начислить(начать(), { lessonId: 'у1', kind: 'vpr', correct: 5, total: 5, состав: [] }, ДАТА).состояние;
  assert.equal(xpДомашек(s), 0);
  assert.deepEqual(s.hwWeeks, {});
});

// Пересдать домашку нельзя, но состояние приходит и из базы, и с другого
// устройства: удвоиться домашние баллы недели не должны и там.
test('повтор домашки недельный счёт не удваивает', () => {
  const событие = { lessonId: 'у1', kind: 'homework', percent: 100, состав: [] };
  const первое = начислить(начать(), событие, ДАТА).состояние;
  const второе = начислить(первое, событие, ДАТА).состояние;

  assert.equal(второе.hwWeeks['2026-W34'], 30);
  assert.equal(xpДомашек(второе), 30);
});

test('домашние баллы разных уроков складываются', () => {
  let s = начислить(начать(), { lessonId: 'у1', kind: 'homework', percent: 100, состав: [] }, ДАТА).состояние;
  s = начислить(s, { lessonId: 'у2', kind: 'homework', percent: 40, состав: [] }, ДАТА).состояние;
  assert.equal(xpДомашек(s), 50);
});
...
test('несуществующая дата не проходит молча', () => {
  // Отказ здесь держится на проверке в предыдущая(); без теста правка соседнего
  // модуля тихо превратила бы ошибку в серию, посчитанную неизвестно от чего.
  assert.throws(() => серия({ '2026-W34': 5 }, new Date('чушь')), /Недопустимый идентификатор недели/);
});
