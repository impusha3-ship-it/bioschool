      ...
      el(
        ...
        [
          ...
          el('span', { class: 'lesson-row__title' }, [
            entry.title,
            // Пометка о работе стоит в файле курса, а не вычисляется из
            // урока: иначе ради шести значков пришлось бы скачать все
            // тридцать четыре. Тест следит, чтобы она не разошлась с уроками.
            // В 9 классе программа ставит практические, а не лабораторные, и
            // называть их чужим словом нельзя: `lab` может быть строкой с
            // видом работы, иначе это лабораторная.
            entry.lab
              ? el(
                  'span',
                  { class: 'lesson-lab' },
                  `(+ ${typeof entry.lab === 'string' ? entry.lab : 'лабораторная'})`,
                )
              : null,
          ]),
          метка(урок),
