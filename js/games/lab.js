 ...
 *
 * Полка показывает картинки, когда каждый её предмет разобран в реестре
 * (`content/inventar.json`, приходит в `inventar`): назван либо прибором со
 * значком, либо образцом, которому значок не нужен. Приборы идут плитками,
 * образцы — подписанными плашками.
 *
 * Образцы без картинки — решение, а не дыра. Луковица на полке никакой
 * загадки не составляет, а что налито в склянку, читают по подписи, и в
 * жизни тоже: две одинаковые склянки (йод и масло в работе про свойства
 * организмов) не различить никак. Узнавать по виду надо инвентарь, и именно
 * он в картинках.
 *
 * Если хоть один предмет в реестре не назван вовсе, полка целиком остаётся
 * названиями: значит, работу ещё не разбирали, и молча показать половину
 * картинками — худшее из двух. По картинке узнают прибор, а по слову рядом
 * читают ответ.
 *
...
export function createLabGame(
  ...
  /*
    Разобрана ли полка — считается один раз при сборке: набор по ходу работы
    не меняется. Реестр может прийти и старой картой (так его отдавали до
    появления образцов) — тогда обходимся ею.
  */
  const полкаЦеликом = набор ? [...(набор.need ?? []), ...(набор.extra ?? [])] : [];
  const реестр = приладить(inventar);
  const картинками = Boolean(
    реестр
      && полкаЦеликом.length
      && полкаЦеликом.every((имя) => реестр.разобран(имя))
      && полкаЦеликом.some((имя) => реестр.значок(имя)),
  );

  function приладить(источник) {
    if (!источник) return null;
    if (источник instanceof Map) {
      return {
        значок: (имя) => источник.get(имя) ?? null,
        разобран: (имя) => источник.has(имя),
      };
    }
    return typeof источник.значок === 'function' ? источник : null;
  }
  ...
  function чип(текст, действие, знак = null, наСтолеЛи = false) {
    ...
    const значок = картинками ? реестр.значок(текст) : null;
    if (значок) {
      ...
      */
      ...
      // Образец на полке с картинками остаётся плашкой с названием: по
      // обобщённому значку его не узнать, а по имени — сразу.
      const образец = картинками ? ' game__chip--specimen' : '';
      btn.className = знак
        ? `game__chip${образец} game__chip--${знак}`
        : `game__chip${образец}`;
