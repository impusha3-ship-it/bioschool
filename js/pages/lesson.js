...
/**
 * Блок заданий тренажёра.
 *
 * Когда задания взяты из настоящей работы, это и говорится: формулировки и
 * способ ответа оттуда, а ученик, впервые увидевший «запишите ответ в виде
 * числа» на самой работе, теряет баллы не на биологии, а на непривычной
 * формулировке. Здесь ошибиться бесплатно и сразу видно, почему.
 *
 * Но по иным темам настоящих заданий в каталоге просто нет, и тогда в блоке
 * стоят свои вопросы по материалу урока. Называть их заданиями ВПР нельзя:
 * метка — утверждение о происхождении, и соврать в ней значит пообещать
 * ученику встречу с тем, чего он на работе не увидит. Поэтому заголовок и
 * подпись блока зависят от того, что в нём лежит.
 */
function renderVpr(вопросы, естьИгра, lesson = null) {
  ...
  const естьНастоящие = вопросы.some((q) => q.source);
  ...
  return el('section', { class: естьИгра ? 'exercise exercise--vpr' : 'exercise' }, [
    ...
    el('h2', { class: 'exercise__head' }, естьНастоящие ? 'Задания в формате ВПР' : 'Задания к уроку'),
    el(
      'p',
      ...
      естьНастоящие
        ...
        : 'Вопросы по материалу урока. Здесь они ничего не стоят: ответил — сразу видно, верно или нет и почему.',
    ),
    quiz.element,
  ]);
}
