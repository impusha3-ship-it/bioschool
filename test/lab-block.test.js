...
/*
  Текстовый ход работы остаётся и при интерактивной версии: по нему работу
  делают в кабинете, если до кабинета дошло. Интерактив её не заменяет, а
  добавляется — и вытесняет только метку «появится позже».
*/
const сИнтерактивом = {
  ...полный,
  run: {
    stages: [
      {
        id: 'shkala',
        prompt: 'Перед тобой мензурка. Как снять показание?',
        result: 'Уровень снят сбоку, на уровне глаз.',
        options: [
          { text: 'Смотреть на уровень сбоку, на уровне глаз', ok: true },
          { text: 'Смотреть сверху', result: 'Сверху уровень кажется выше, чем есть.' },
        ],
      },
    ],
  },
...
test('у работы с интерактивом метка «появится позже» уходит', () => {
  const doc = makeFakeDocument();
  const node = renderBlock(сИнтерактивом, { document: doc });

  assert.ok(!textOf(node).includes('появится позже'), 'метка осталась, хотя работа уже интерактивная');
  assert.ok(allByTag(node, 'ol').length >= 1, 'текстовый ход работы должен остаться');
});

test('сломанный интерактив не роняет конспект, а возвращает прежнюю метку', () => {
  const doc = makeFakeDocument();
  const node = renderBlock({ ...полный, run: { stages: [] } }, { document: doc });

  assert.equal(node.className, 'lab');
  assert.ok(textOf(node).includes('позже'), 'при негодном интерактиве должна остаться прежняя метка');
});
