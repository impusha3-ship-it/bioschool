...
// 2026 год выбран не случайно: в нём 53 недели (год начинается в четверг), и
// именно на этой границе ломается наивная реализация ISO-недели.
test('31 декабря 53-недельного года остаётся в его последней неделе', () => {
  assert.equal(неделя(new Date(2026, 11, 31)), '2026-W53');
});

test('1 января следующего года всё ещё в 53-й неделе предыдущего', () => {
  assert.equal(неделя(new Date(2027, 0, 1)), '2026-W53');
});

test('предыдущая перед первой неделей 2027 — 53-я неделя 2026', () => {
  assert.equal(предыдущая('2027-W01'), '2026-W53');
});

test('битый идентификатор бросает ошибку, а не выдаёт NaN-неделю', () => {
  assert.throws(() => предыдущая('2026-34'), /Недопустимый идентификатор недели/);
});

test('номер недели вне диапазона тоже бросает ошибку', () => {
  assert.throws(() => предыдущая('2026-W00'), /Недопустимый идентификатор недели/);
});
