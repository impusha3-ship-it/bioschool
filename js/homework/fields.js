...
export function questionField(q, ответы = {}, настройки = {}) {
  ...
  // У заданий, взятых из настоящих работ, в пометке стоит год и вариант.
  // Это не украшение: ученик видит, что решает не выдумку, а то, что уже
  // стояло в работе, — и знает, из какой именно работы это задание.
  const пометка = q.type === 'open' ? 'Проверяет учитель' : (q.source ?? q.exam);
